import json
import re
import subprocess
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PDF_DIR = ROOT / 'source-pdfs'
OUT = ROOT / 'src' / 'data' / 'questions.ts'
TEXT_DIR = ROOT / 'build' / 'text'
TEXT_DIR.mkdir(parents=True, exist_ok=True)

EXAMS = [
    {
        'id': 'r7', 'year': '令和7年度', 'label': '2025 / 令和7年度',
        'pdf': 'R7_question_answer.pdf',
        'url': 'https://www.retio.or.jp/wp-content/uploads/2025/12/R7_question_answer.pdf',
        'mode': 'text', 'passScore': 33,
        'explanationBaseUrl': 'https://takken-success.info/2025kakomon/r07-',
    },
    {
        'id': 'r6', 'year': '令和6年度', 'label': '2024 / 令和6年度',
        'pdf': 'R6_question_answer.pdf',
        'url': 'https://www.retio.or.jp/wp-content/uploads/2025/03/R6_question_answer.pdf',
        'mode': 'text', 'passScore': 37,
        'explanationBaseUrl': 'https://takken-success.info/2024kakomon/r06-',
    },
    {
        'id': 'r5', 'year': '令和5年度', 'label': '2023 / 令和5年度',
        'pdf': 'R5_question_answer.pdf',
        'url': 'https://www.retio.or.jp/wp-content/uploads/2025/03/R5_qestion_answer%E3%80%80.pdf',
        'mode': 'text', 'passScore': 36,
        'explanationBaseUrl': 'https://takken-success.info/2023kakomon/r05-',
    },
    {
        'id': 'r4', 'year': '令和4年度', 'label': '2022 / 令和4年度',
        'pdf': 'R4_question_answer.pdf',
        'url': 'https://www.retio.or.jp/wp-content/uploads/2024/10/R4-q_a.pdf',
        'mode': 'ocr', 'passScore': 36,
        'explanationBaseUrl': 'https://takken-success.info/2022kakomon/r04-',
    },
    {
        'id': 'r3_12', 'year': '令和3年度12月', 'label': '2021 / 令和3年度12月',
        'pdf': 'R3_12_question_answer.pdf',
        'url': 'https://www.retio.or.jp/wp-content/uploads/2024/12/R3-question_002.pdf',
        'mode': 'ocr', 'passScore': 34,
        'explanationBaseUrl': 'https://takken-success.info/2021-12kakomon/r032-',
    },
]

ANSWER_OVERRIDES = {
    'r7': [3,3,3,4,4,1,1,2,1,3,3,3,3,1,4,4,2,2,2,4,4,4,1,2,1,4,1,2,2,3,4,2,3,3,1,4,4,3,4,3,1,2,4,2,4,2,3,2,1,1],
    'r6': [1,4,3,4,2,4,1,1,2,4,3,3,1,3,4,1,2,2,3,2,1,4,2,2,3,3,4,2,4,4,1,3,3,3,2,4,3,4,4,2,1,2,4,1,2,1,4,1,2,3],
    'r5': [1,1,2,4,4,3,3,3,2,3,4,3,2,2,4,1,3,1,1,4,2,1,1,4,4,3,4,3,2,1,4,4,1,3,4,3,3,2,2,4,2,3,4,1,4,2,2,1,2,3],
    'r4': [3,3,4,1,2,3,4,3,1,2,3,1,1,2,3,2,3,3,4,1,4,3,3,2,2,2,1,1,3,3,1,1,2,4,4,1,2,4,4,2,2,2,2,4,3,1,4,'ALL',2,4],
    'r3_12': [4,3,2,4,3,1,4,2,3,1,3,2,2,2,4,3,3,2,1,1,4,1,2,1,2,3,4,1,3,3,2,1,2,1,4,4,2,3,3,2,1,3,1,[2,3],4,1,4,4,2,4],
}

CATEGORY = [
    (1, 14, '権利関係'),
    (15, 22, '法令上の制限'),
    (23, 25, '税・価格評定'),
    (26, 45, '宅建業法'),
    (46, 50, '免除科目'),
]

OCR_KANJI_FIXES = [
    ('瑕 疵 担保', '瑕疵担保'),
    ('瑕 疵', '瑕疵'),
    ('瑕疵 担保', '瑕疵担保'),
    ('市 街 化 区 域 内 に 所 在 す る 一 団 の 土 地 で あ る 甲 土 地', '市街化区域内に所在する一団の土地である甲土地'),
    ('乙 土 地', '乙土地'),
    ('面 積', '面積'),
    ('） と 乙土地', '）と乙土地'),
    ('面積1,300', '面積 1,300'),
    ('令和 5 年3 月末', '令和 5 年 3 月末'),
    ('令和 4 年 3月末', '令和 4 年 3 月末'),
    ('宅地建惣', '宅地建物'),
    ('宅地玩取引', '宅地建物取引'),
    ('宅地駐は建物', '宅地又は建物'),
    ('和宅地', '宅地'),
    ('取引上の登録', '取引士の登録'),
    ('取引十', '取引士'),
    ('取引土', '取引士'),
    ('取引寺', '取引士'),
    ('撮導担保', '瑕疵担保'),
    ('明導担保', '瑕疵担保'),
    ('直久担保', '瑕疵担保'),
    ('下六担保', '瑕疵担保'),
    ('下獲担保', '瑕疵担保'),
    ('下導担保', '瑕疵担保'),
    ('下獲が', '瑕疵が'),
    ('表導担保', '瑕疵担保'),
    ('表六揚保次任', '瑕疵担保責任'),
    ('保険有約', '保険契約'),
    ('資任', '責任'),
    ('均任', '責任'),
    ('次任', '責任'),
    ('措胃', '措置'),
    ('放ずる', '講ずる'),
    ('現定', '規定'),
    ('人貸借', '賃貸借'),
    ('表示きれて', '表示されて'),
    ('閲覧に供き', '閲覧に供さ'),
    ('供さされる', '供される'),
    ('前金の刑', '罰金の刑'),
    ('消除きれ', '消除され'),
    ('差づく', '基づく'),
    ('告げばられた', '告げられた'),
    ('当謎', '当該'),
    ('近有隣', '近隣'),
    ('直起距離', '直線距離'),
    ('当設旧跡', '当該旧跡'),
    ('未において', '末において'),
    ('信和', '令和'),
    ('土地販書', '土地白書'),
    ('花沿岩', '花崗岩'),
    ('免んる', '免れる'),
    ('璧厚', '壁厚'),
    ('耐岩', '耐震'),
    ('補強政良', '補強改良'),
    ('奇量', '重量'),
    ('閉物', '建物'),
    ('名徳', '名簿'),
    ('名稚', '名簿'),
    ('逢する', '達する'),
    ('に係るる', 'に係るものである'),
    ('婚敵歴', '婚姻歴'),
    ('賀かなければ', '置かなければ'),
    ('封ら', '執ら'),
    ('天り', '張り'),
    ('委内所', '案内所'),
    ('超算', '起算'),
    ('避日', '翌日'),
    ('直反', '違反'),
    ('間集広告', '募集広告'),
    ('問集広告', '募集広告'),
    ('耐月診断', '耐震診断'),
    ('粘果', '結果'),
    ('当恋', '当該'),
    ('人権', '債権'),
    ('供託双は', '供託又は'),
    ('十水', '雨水'),
    ('賠交', '瑕疵'),
    ('不喘合', '不適合'),
    ('宅地寺物', '宅地建物'),
    ('取引玉者', '取引業者'),
    ('和宅地填物', '宅地建物'),
    ('宅地填物', '宅地建物'),
    ('宅地叉は建物', '宅地又は建物'),
    ('就作する', '就任する'),
    ('と県に移転', '乙県に移転'),
    ('内県知事', '丙県知事'),
    ('成県知事', '戊県知事'),
    ('経包する', '経過する'),
    ('億地権', '借地権'),
    ('減失', '滅失'),
    ('宅地玩取引寺', '宅地建物取引士'),
    ('資格計険', '資格試験'),
    ('宅地域物', '宅地建物'),
    ('宅地道物', '宅地建物'),
    ('宅地始物', '宅地建物'),
    ('宅地壮物', '宅地建物'),
    ('宅地針物', '宅地建物'),
    ('信則', '罰則'),
    ('記名押印をきせる', '記名押印させる'),
    ('就作', '就任'),
    ('劣るあものの', '劣るものの'),
    ('礎式構造', '組積式構造'),
    ('いずれや正解', 'いずれも正解'),
    ('設間文', '設問文'),
    ('間44', '問44'),
    ('当訪', '当該'),
    ('正しいもゃのは', '正しいものは'),
    ('正しいもるの', '正しいもの'),
    ('正しいゃの', '正しいもの'),
    ('正しいゃもの', '正しいもの'),
    ('正 しいゃの', '正しいもの'),
    ('正しいちもの', '正しいもの'),
    ('正しいちの', '正しいもの'),
    ('いえないも', 'いずれも'),
    ('作成されたもるの', '作成されたもの'),
    ('契約午', '契約書'),
    ('誤渡', '譲渡'),
    ('賃賃借', '賃貸借'),
    ('賃貸億', '賃貸借'),
    ('がきれていた', 'がされていた'),
    ('名介', '媒介'),
    ('召介', '媒介'),
    ('和鑑定', '鑑定'),
    ('角定評価', '鑑定評価'),
    ('国圭', '国土'),
    ('弄則', '罰則'),
    ('都首府県知事', '都道府県知事'),
    ('当玉', '当該'),
    ('割又販売', '割賦販売'),
    ('人金額', '金額'),
    ('借和賃', '借賃'),
    ('特別懲収', '特別徴収'),
    ('徹課期日', '賦課期日'),
    ('周定資産', '固定資産'),
    ('回定資産', '固定資産'),
    ('表是登記', '表題登記'),
    ('北舎で', '校舎で'),
    ('車込み', '申込み'),
    ('街げられて', '告げられて'),
    ('分聞', '分間'),
    ('薬法違反', '業法違反'),
    ('取糸法', '取引業法'),
    ('と建物の譲渡契約', '乙建物の譲渡契約'),
    ('土地氏定', '土地鑑定'),
    ('官報で全ホ', '官報で公示'),
    ('選住用', '居住用'),
    ('その和紀を', 'その旨を'),
    ('人怠った', '怠った'),
    ('必ずしゃ', '必ずしも'),
    ('倍地上', '借地上'),
    ('倍地権', '借地権'),
    ('当訪知 .反', '当該違反'),
    ('登記答', '登記簿'),
    ('着しく', '著しく'),
    ('承語して', '承諾して'),
    ('土地付和物', '土地付建物'),
    ('売買物 .件', '売買物件'),
    ('六約金', '違約金'),
    ('できない陣を', 'できない旨を'),
    ('肖条件', '当該条件'),
    ('消減', '消滅'),
    ('市衡化', '市街化'),
    ('市衡地', '市街地'),
    ('倍反', '違反'),
    ('條反', '違反'),
    ('信反', '違反'),
    ('佑反', '違反'),
    ('賃僅人', '賃借人'),
    ('転僅人', '転借人'),
    ('賃供人', '賃借人'),
    ('借地億家', '借地借家'),
    ('倍地借家', '借地借家'),
    ('契約午', '契約書'),
    ('鞍結', '締結'),
    ('宅地寺取引業者', '宅地建物取引業者'),
    ('宅地刈物', '宅地建物'),
    ('宅地癌物', '宅地建物'),
    ('取引圭証', '取引士証'),
    ('登記箸', '登記簿'),
    ('その軒', 'その旨'),
    ('有有効', '有効'),
    ('自らら', '自ら'),
    ('締粘', '締結'),
    ('軽減措持', '軽減措置'),
    ('者道府県知事', '都道府県知事'),
    ('調っている', '誤っている'),
    ('謀っている', '誤っている'),
    ('どれが', 'どれか'),
    ('家許義判所', '家庭裁判所'),
    ('家庭半判所', '家庭裁判所'),
    ('放乗', '放棄'),
    ('放楽', '放棄'),
    ('披相続人', '被相続人'),
    ('遺罰分', '遺留分'),
    ('禁鋼', '禁錮'),
]

def run_pdftotext(pdf: Path) -> str:
    return subprocess.check_output(['pdftotext', '-layout', str(pdf), '-'], text=True, errors='replace')

def fix_ocr_kanji(text: str) -> str:
    for old, new in OCR_KANJI_FIXES:
        text = text.replace(old, new)
    text = re.sub(r'宅地[填針閉値寺旬始玉各培鑑才釘道域辻道始壮針]物', '宅地建物', text)
    text = re.sub(r'「『([^」\n]{1,40})]\s*」', r'「\1」', text)
    text = re.sub(r'「([^」\n]{1,40})]\s*」', r'「\1」', text)
    text = re.sub(r'「([^」\n]{1,40})]', r'「\1」', text)
    text = text.replace('「法|」', '「法」')
    # Repair common OCR mistakes in suru-verb passive forms.
    verbal_nouns = (
        r'登記|媒介|説明|表示|記載|課|提出|受託|処分|公示|指定|確認|実施|設置|締結|変更|適用|認可|完了|管理|使用|収益|賃借|'
        r'評価|譲渡|処理|請求|返還|交付|送付|承諾|開始|構築|築造|提供|廃止|解散|解除|依頼|供|吸収|公告|売却|購入|取得|登録|届出|免許|取消|'
        r'命令|勧告|指導|助言|要請|報告|検査|監査|調査|査定|鑑定|判定|審査|認定|証明|発行|作成|調製|記録|保存|保管|運営|経営|'
        r'活動|執行|履行|遵守|違反|駐車|制限'
    )
    def repair_passive_forms(block: str) -> str:
        return re.sub(rf'({verbal_nouns})きれ(?!る|た)', r'\1され',
            re.sub(rf'({verbal_nouns})きれた', r'\1された',
            re.sub(rf'({verbal_nouns})きれる', r'\1される', block)))

    question_blocks = re.split(r'【問', text)
    if len(question_blocks) > 1:
        text = question_blocks[0] + '【問' + '【問'.join([
            repair_passive_forms(block)
            for block in question_blocks[1:]
        ])
    else:
        text = repair_passive_forms(text)
    return text

def clean_text(text: str, collapse_space_runs: bool = True) -> str:
    text = text.replace('\r\n', '\n').replace('\r', '\n')
    text = fix_ocr_kanji(text)
    text = re.sub(r'\n\s*[-ー―]?[ 　]*\d+[ 　]*[-ー―]?\s*\n', '\n', text)
    text = re.sub(r'AB\.indd.*\n', '', text)
    text = re.sub(r'Syntax Warning:.*\n', '', text)
    if collapse_space_runs:
        text = re.sub(r'[ 　]{2,}', ' ', text)
    text = re.sub(r'[ \t]+\n', '\n', text)
    text = re.sub(r'\n{3,}', '\n\n', text)
    return text.strip()

def format_display_text(text: str) -> str:
    text = clean_text(text, collapse_space_runs=False)
    text = re.sub(r'【問\s*(\d{1,2})】\s*', r'【問 \1】 ', text)
    text = re.sub(r'\n+', '\n', text)
    raw_lines = [line for line in text.splitlines() if line.strip()]
    lines = [(line, re.sub(r'[ 　]{2,}', ' ', line.strip())) for line in raw_lines]
    chunks = []
    current = ''
    choice_indent = choice_block_indent(raw_lines)

    for raw_line, line in lines:
        indent = len(raw_line) - len(raw_line.lstrip(' 　\t'))
        is_question = line.startswith('【問')
        is_choice = is_choice_line(line, indent, choice_indent)
        is_marker = re.match(r'^[（(](?:判決文|資料|注|図|表)', line) is not None

        if is_question or is_choice or is_marker:
            if current:
                chunks.append(current.strip())
            current = line
        else:
            current = f'{current}{line}' if current else line

    if current:
        chunks.append(current.strip())

    formatted = fix_ocr_kanji('\n\n'.join(chunks))
    formatted = formatted.replace(']', '')
    formatted = re.sub(r'。 [0-9](?=\n\n)', '。', formatted)
    return formatted.strip()

def line_indent(line: str) -> int:
    return len(line) - len(line.lstrip(' 　\t'))

def choice_block_indent(lines) -> int | None:
    candidates = []
    for idx, line in enumerate(lines):
        stripped = line.strip()
        match = re.match(r'^([1-4])[ 　\s]+', stripped)
        if match:
            candidates.append((idx, line_indent(line), int(match.group(1))))

    for indent in sorted({candidate[1] for candidate in candidates}):
        expected = 1
        for _, candidate_indent, number in candidates:
            if candidate_indent == indent and number == expected:
                expected += 1
                if expected == 5:
                    return indent

    return None

def is_choice_line(line: str, indent: int, choice_indent: int | None) -> bool:
    if choice_indent is None or indent != choice_indent:
        return False
    return re.match(r'^[1-4]\s+', line) is not None

def category_for(n: int) -> str:
    for start, end, label in CATEGORY:
        if start <= n <= end:
            return label
    return 'その他'

def normalize_question_markers(text: str) -> str:
    # Keep clean markers from text PDFs, and repair common OCR variants from official scanned PDFs.
    text = text.replace('【間', '【問').replace('[問', '【問').replace('〔問', '【問')
    text = text.replace(']】', '】').replace('）】', '】').replace(')】', '】')
    text = re.sub(r'[【\[\(（]?\s*問\s*(\d{1,2})\s*[】\]\)）]', r'【問 \1】', text)
    text = re.sub(r'[【\[\(（]\s*問\s*(\d{1,2})\s*[】\]\)）]?', r'【問 \1】', text)

    # OCR sometimes reads question markers as "[Fl 17】", "(fl 49】", or "RL 10】".
    marker_codes = r'FH|FM|FA|Fi|Fl|fl|ft|Hl|RL|A|i|I'
    text = re.sub(
        rf'(?m)(^|[\n\f])\s*[|:.\-。ー―]*\s*(?:[\[【(（]\s*)?(?:{marker_codes})\]?\s*(\d{{1,2}})\s*[\]】\)]',
        r'\1【問 \2】',
        text,
    )
    text = re.sub(
        rf'[\[【(（]?\s*(?:{marker_codes})\s*(?:問|間)\s*(\d{{1,2}})\s*[\]】\)]',
        r'【問 \1】',
        text,
    )
    text = re.sub(r'【+問\s*(\d{1,2})[\]】\)]', r'【問 \1】', text)
    return text

def remove_exemption_notice(text: str) -> str:
    # The official PDFs include an instruction that contains question markers
    # such as "【問 46】から【問 50】まで"; it is not a question block.
    return re.sub(
        r'以下の\s*【問\s*46】から\s*【問\s*50】までについては、\s*'
        r'登録講習修了者としての受験申込みを\s*'
        r'受理された方は、免除となりますので、解答しないでください。',
        '',
        text,
    )

def parse_questions(text: str, exam_id: str):
    text = normalize_question_markers(clean_text(text, collapse_space_runs=False))
    text = remove_exemption_notice(text)
    matches = list(re.finditer(r'【問\s*(\d{1,2})】', text))
    items = []
    seen = set()
    for idx, m in enumerate(matches):
        n = int(m.group(1))
        if n < 1 or n > 50 or n in seen:
            continue
        end = matches[idx + 1].start() if idx + 1 < len(matches) else len(text)
        block = clean_text(text[m.start():end], collapse_space_runs=False)
        # Remove answer table and back matter if it was captured after Q50.
        block = re.split(
            r'◆合格判定基準|◆試験問題の正解番号|受[ 　]*験[ 　]*番[ 　]*号|正解番号表|合否判定基準|ー[ 　]*95[ 　]*一',
            block
        )[0].strip()
        block = re.sub(r'【\s*$', '', block).strip()
        block = format_display_text(block)
        if len(block) < 40:
            continue
        items.append({'number': n, 'text': block})
        seen.add(n)
    return items

def official_pdf_fallback(n: int) -> str:
    return f'公式PDFの問{n}を確認して解答してください。OCR抽出でこの問題文を安定取得できなかったため、原本PDF確認を前提にしています。'

def is_unstable_ocr_text(text: str) -> bool:
    long_latin_words = [
        word for word in re.findall(r'[A-Za-z]{4,}', text)
        if word not in {'SNS'}
    ]
    residual_ocr_terms = re.search(
        r'正しいもる|正しいゃ|市衡|倍反|條反|信反|佑反|消減|賃僅|転僅|賃供|'
        r'宅地[填針閉値寺旬始玉各培鑑才釘道域辻壮]物|取引[十土寺玉]|'
        r'あB[at]',
        text,
    )
    symbol_noise = re.search(r'[=~_|]|ー{2,}', text)
    return bool(long_latin_words or residual_ocr_terms or symbol_noise)

def fallback_by_pages(exam_id: str, text: str):
    # OCR marker repair is imperfect. For missing years, still create all answerable cards,
    # attaching the nearest raw OCR excerpt if available and always linking to the official PDF.
    if exam_id in {'r4', 'r3_12'}:
        return [{'number': n, 'text': official_pdf_fallback(n)} for n in range(1, 51)]

    parsed = {q['number']: q['text'] for q in parse_questions(text, exam_id)}
    fallback = []
    for n in range(1, 51):
        body = parsed.get(n)
        if not body:
            body = official_pdf_fallback(n)
        elif is_unstable_ocr_text(body):
            body = official_pdf_fallback(n)
        else:
            body = format_display_text(body)
        fallback.append({'number': n, 'text': body})
    return fallback

def answer_to_list(value):
    if value == 'ALL':
        return [1,2,3,4]
    if isinstance(value, list):
        return value
    return [value]

def build_dataset():
    all_questions = []
    exam_summaries = []
    for exam in EXAMS:
        pdf = PDF_DIR / exam['pdf']
        if exam['mode'] == 'ocr':
            text_path = ROOT / 'build' / 'ocr' / f"{pdf.stem}.txt"
            text = text_path.read_text(encoding='utf-8')
            questions = fallback_by_pages(exam['id'], text)
        else:
            text = run_pdftotext(pdf)
            (TEXT_DIR / f"{exam['id']}.txt").write_text(text, encoding='utf-8')
            parsed = parse_questions(text, exam['id'])
            by_num = {q['number']: q for q in parsed}
            questions = []
            for n in range(1, 51):
                if n in by_num:
                    questions.append({
                        **by_num[n],
                        'text': format_display_text(by_num[n]['text']),
                    })
                else:
                    questions.append({'number': n, 'text': f'公式PDFの問{n}を確認して解答してください。'})
        answers = ANSWER_OVERRIDES[exam['id']]
        extracted_count = sum(1 for q in questions if not q['text'].startswith('公式PDFの問'))
        exam_summaries.append({
            'id': exam['id'], 'year': exam['year'], 'label': exam['label'], 'sourceUrl': exam['url'],
            'passScore': exam['passScore'], 'questionCount': 50, 'extractedCount': extracted_count,
        })
        for q in questions:
            n = q['number']
            ans = answer_to_list(answers[n - 1])
            all_questions.append({
                'id': f"{exam['id']}-{n:02d}",
                'examId': exam['id'],
                'year': exam['year'],
                'label': exam['label'],
                'number': n,
                'category': category_for(n),
                'sourceUrl': exam['url'],
                'externalExplanationName': '宅建レトス',
                'externalExplanationUrl': f"{exam['explanationBaseUrl']}{n}/",
                'questionText': q['text'],
                'correctChoices': ans,
                'isAllCorrect': len(ans) == 4,
                'officialExplanation': (
                    '公式PDFの正解番号表: 全ての選択肢を正解扱い。'
                    if len(ans) == 4 else
                    ('公式PDFの正解番号表: 正解 ' + ' / '.join(map(str, ans)))
                ),
            })
    return exam_summaries, all_questions

def write_typescript(exam_summaries, all_questions):
    OUT.parent.mkdir(parents=True, exist_ok=True)
    content = "export type TakkenQuestion = {\n"
    content += "  id: string;\n  examId: string;\n  year: string;\n  label: string;\n  number: number;\n  category: string;\n  sourceUrl: string;\n  externalExplanationName: string;\n  externalExplanationUrl: string;\n  questionText: string;\n  correctChoices: number[];\n  isAllCorrect: boolean;\n  officialExplanation: string;\n};\n\n"
    content += "export type TakkenExam = {\n  id: string;\n  year: string;\n  label: string;\n  sourceUrl: string;\n  passScore: number;\n  questionCount: number;\n  extractedCount: number;\n};\n\n"
    content += "export const takkenExams: TakkenExam[] = " + json.dumps(exam_summaries, ensure_ascii=False, indent=2) + ";\n\n"
    content += "export const takkenQuestions: TakkenQuestion[] = " + json.dumps(all_questions, ensure_ascii=False, indent=2) + ";\n"
    OUT.write_text(content, encoding='utf-8')

def main():
    exam_summaries, all_questions = build_dataset()
    write_typescript(exam_summaries, all_questions)
    print(json.dumps({'exams': exam_summaries, 'questions': len(all_questions)}, ensure_ascii=False, indent=2))

if __name__ == '__main__':
    main()
